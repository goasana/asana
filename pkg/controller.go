...
func (c *Controller) Trace() {
	...
	c.Ctx.SetHeader("Content-Type", "message/http")
	c.Ctx.SetHeader("Content-Length", fmt.Sprint(len(hs)))
	c.Ctx.SetHeader("Cache-Control", "no-cache, no-store, must-revalidate")
	c.Ctx.Text(hs)
}
...
func (c *Controller) ServeFormatted(encoding ...bool) {
	hasIndent := BConfig.RunMode != PROD
	hasEncoding := len(encoding) > 0 && encoding[0]
	c.Ctx.ServeFormatted(hasIndent, hasEncoding)
}

// Input returns the input data map from POST or PUT request body and query string.
func (c *Controller) Input() url.Values {
	if c.Ctx.HTTPRequest.Form == nil {
		c.Ctx.HTTPRequest.ParseForm()
	}
...
func (c *Controller) SessionRegenerateID() {
	...
	c.Ctx.CruSession = c.CruSession
}

// DestroySession cleans session data and session cookie.
func (c *Controller) DestroySession() {
	c.Ctx.CruSession.Flush()
	c.Ctx.CruSession = nil
