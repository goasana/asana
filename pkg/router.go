...
func (p *ControllerRegister) GetContext() *asanacontext.Context {
	return p.pool.Get().(*asanacontext.Context)
}

// GiveBackContext put the ctx into pool so that it could be reuse
func (p *ControllerRegister) GiveBackContext(ctx *asanacontext.Context) {
	p.pool.Put(ctx)
}
...
func (p *ControllerRegister) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	context.EnableGzip = BConfig.EnableGzip

	if BConfig.RunMode == DEV {
		context.SetHeader("Server", BConfig.ServerName)
	}
	...
	if r.Method != http.MethodGet && r.Method != http.MethodHead {
		if BConfig.CopyRequestBody && !context.IsUpload() {
			...
			context.CopyBody(BConfig.MaxMemory)
		}
		context.ParseFormOrMultiForm(BConfig.MaxMemory)
	}

	// session init
	if BConfig.WebConfig.Session.SessionOn {
		var err error
		context.CruSession, err = GlobalSessions.SessionStart(rw, r)
		if err != nil {
			logs.Error(err)
		...
		defer func() {
			if context.CruSession != nil {
				context.CruSession.SessionRelease(rw)
			}
		}()
	}
	...
	if context.RunController != nil && context.RunMethod != "" {
		findRouter = true
		runMethod = context.RunMethod
		runRouter = context.RunController
	} else {
	...
	if splat := context.Param(":splat"); splat != "" {
		for k, v := range strings.Split(splat, "/") {
			context.SetParam(strconv.Itoa(k), v)
		}
	}

	if routerInfo != nil {
		// store router pattern into context
		context.SetFlash("RouterPattern", routerInfo.pattern)
	}
	...
	if routerInfo != nil {
		if routerInfo.routerType == routerTypeRESTFul {
			...
			isRunnable = true
			routerInfo.handler.ServeHTTP(context.ResponseWriter, context.HTTPRequest)
		} else {
			runRouter = routerInfo.controllerType
			methodParams = routerInfo.methodParams
			method := r.Method
			if r.Method == http.MethodPost && context.Query("_method") == http.MethodPut {
				method = http.MethodPut
			}
			if r.Method == http.MethodPost && context.Query("_method") == http.MethodDelete {
	...
	if !isRunnable {
		...
		if BConfig.WebConfig.EnableXSRF {
			...
			if r.Method == http.MethodPost || r.Method == http.MethodDelete || r.Method == http.MethodPut ||
				(r.Method == http.MethodPost && (context.Query("_method") == http.MethodDelete || context.Query("_method") == http.MethodPut)) {
		...
		if !context.ResponseWriter.Started {
			...
			// render template
			if !context.ResponseWriter.Started && context.GetStatus() == 0 {
...
Admin:
	...
	if context.GetStatus() != 0 {
		context.ResponseWriter.WriteHeader(context.GetStatus())
	}
}
