...
func TestCheck(t *testing.T) {
	if exists := safeMap.Check("GNURub"); !exists {
		t.Error("expected", true, "got", false)
	}
}

func TestGet(t *testing.T) {
	if val := safeMap.Get("GNURub"); val.(int) != 1 {
		t.Error("expected value", 1, "got", val)
	}
}

func TestDelete(t *testing.T) {
	safeMap.Delete("GNURub")
	if exists := safeMap.Check("GNURub"); exists {
...
func TestItems(t *testing.T) {
	safeMap := NewBeeMap()
	safeMap.Set("GNURub", "hello")
	for k, v := range safeMap.Items() {
		key := k.(string)
		value := v.(string)
		if key != "GNURub" {
			t.Error("expected the key should be GNURub")
		}
		if value != "hello" {
